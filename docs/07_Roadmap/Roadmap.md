...
- [x] **Seguimiento 2026-08-10 — respondido por el usuario:** `Consolidado`/`Cross_docking` no dependen de terminal (igual que el contrato original, por `idUbicacion` + producto) y `Gastos_THC` sí depende de la naviera. `TarifaConsolidado`/`TarifaCross_docking` (o sin el prefijo `Tarifa`) entran al acumulador de `leerTarifaSitioMaestroNuevo()`, que pasó de 5 a 7 conceptos. `Gastos_THC` se lee hacia una tabla nueva, `DatosEntrada.TarifaThc` (clave naviera + tipo de contenedor), con su propio resolvedor y un overload de `thcUsdContenedor()`; `Main.thcUsdContenedorPedido()` decide qué overload usar y reemplaza los tres call sites que cobraban THC.
- [x] **Bug preexistente de ADR-067 encontrado y corregido de paso:** `coberturaTarifas()`/`unaSolaRoundTrip()` llamaban `producto(producto).tipoContenedor` con un solo argumento — no compilaba, porque `producto()` sólo existe de a dos (`producto`, `material`) desde ADR-067. Nuevo método `tipoContenedorDe(producto)` lo resuelve vía el primer material de `materialesDe()`, con fallo explícito si dos materiales de un mismo producto declaran tipos de contenedor distintos.
- [x] Proveedores de `Gastos_THC` que no son una naviera real del enum (`FORWARDER`, `SILVERFREIGHT`) se descartan con `advertencias`, no abortan el import — en los datos reales el 100% de `PedidoPlan` declara `MAERSK`, que sí está en el enum.
- [x] Confirmado con la ronda 3 de `Maestro_Simulacion.xlsx`: `GRUPO_PAZ`/`CONTROL_UNION` ya tienen filas en `TarifaFleteCamionproducto`/`TarifaRoundTrip` (bloqueaba la ronda 2, resuelto por el usuario) y la columna `material` de `ProduccionPlan`/`PedidoPlan` llegó en minúscula (antes `Material`, corregido por el usuario).
...
## 11q. MOD — El material sale en `costos_eventos` y `asignaciones_elegidas` (ADR-071)

Implementado en `RedLogistica_Exportacion.alp` y `model_src/` (esquema `ADR-064.2`). Motivado por un reporte contable pedido por el usuario (`Resumen_cuentas.xlsx`, aging por material en los mismos 12 tramos de días que ya usan las hojas de tarifa) que necesita separar `JUGO` en `JCL`/`JCCL`/`PCL`, y ninguna tabla de auditoría exportaba `material` (ADR-067) — sólo `producto`.

- [x] `AsignacionPedido` gana el campo `material` (seteado en `crearAsignacion()` desde `pedido.material`), columna nueva junto a `producto` en `encabezadoCsv()`/`toCsv()`.
- [x] `Main.exportarCostosEventos()` resuelve `material` por contenedor (`materialDeContenedor`, mismo patrón que `asignacionDeContenedor`), por lote (`materialDeLote`, contra la población `lotes`) o, como último fallback, por pedido (`materialDePedido`, contra `pedidos`), sin agregar el campo a `RegistroCostos.Cargo` ni tocar sus 15 sitios de `registro.registrar(...)`. Los cargos de alcance `RED` quedan con `material = ""`.
- [x] Fallback por pedido agregado tras correr el modelo parchado contra un escenario real: sin él, 706 900 USD de `FLETE_PRODUCTO` (flete por viaje sin contenedor ni lote todavía) quedaban con `material` vacío y desaparecían del reporte contable sin avisar.
- [x] `AuditoriaRed.VERSION_ESQUEMA` de `ADR-064.1` a `ADR-064.2`; `costos_eventos.csv` de 27 a 28 columnas, `asignaciones_elegidas.csv` de 30 a 31.
