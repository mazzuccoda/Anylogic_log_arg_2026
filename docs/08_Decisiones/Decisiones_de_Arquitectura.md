...
Antes de dar la fase por terminada (`docs/07_Roadmap/Roadmap.md` §17) falta: compilar en el IDE; cargar al menos un par de filas depósito→depósito en `TarifaFleteProducto`/`Distancia` (dirección exacta — `tarifaFlete()` no es simétrica, hace falta la fila `origen=X, destino=Y` para el sentido que se quiere habilitar); y correr `V-COST-12` y `V-COST-13` (`docs/06_Validacion/Plan_de_Validacion.md`) con esos datos cargados.
