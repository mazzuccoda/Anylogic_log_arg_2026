...
De los conceptos que `TarifaSitio` unificaba, tres perdieron una ambigüedad real al migrar: `Consolidado` trae valores distintos por terminal de destino (no sólo por sitio y producto, que es la clave que tiene `consolidacion_tarifa` hoy), `Cross_docking` tiene la misma forma, y `Gastos_THC` está por naviera y tipo de contenedor, sin sitio — el pedido ya trae `naviera`, pero incorporar esa dimensión a `thc_usd_contenedor` es un cambio de modelo, no de importador. Se preguntó explícitamente al usuario cómo interpretar estas tres hojas y la pregunta no se respondió; adivinar mal ahí produce un costo de campaña incorrecto sin que se note, así que quedan explícitamente sin leer (ver Consecuencias).
...
6. **`Consolidado`, `Cross_docking` y `Gastos_THC` no se leen.** `consolidacion_tarifa`, `cross_dock_tarifa` y `thc_usd_contenedor` quedan en `0` para todo sitio y producto, con una `advertencia` explícita en `ultimasAdvertencias` que nombra las tres hojas y la razón. No es un dato faltante silencioso: está declarado, visible, y apunta a la decisión pendiente.

**Alternativas:** convertir el libro nuevo al formato viejo con un script aparte antes de importar (descartada — dos formatos de verdad en paralelo, y cualquier corrección posterior en el maestro real habría que volver a convertir); resolver los buckets dentro de `DatosEntrada` en vez de expandirlos al leer (descartada — reescribe `Tarifa.vigente()` y `coberturaTarifas()` para una segunda representación de vigencia, sin necesidad: expandir a filas por mes reusa toda la lógica existente sin tocarla); adivinar consolidación por terminal y THC por naviera con el criterio que parecía más razonable (descartada explícitamente — son cambios de modelo que afectan el costo real de la campaña, y la pregunta hecha al usuario no se respondió; queda como limitación declarada, no como bug).

**Consecuencias:** con el libro nuevo, la carga ya no aborta por hoja faltante — pero **el costo de campaña queda incompleto** mientras Consolidado/Cross_docking/Gastos_THC no se resuelvan: toda estiba, cross dock y THC se factura en USD 0. Cualquier corrida con este libro sirve para probar `material` (ADR-067) y la estructura general, pero **no** es comparable en costo total contra una corrida con el contrato original hasta que se resuelvan esas tres hojas. Además, `GRUPO_PAZ` y `CONTROL_UNION` (agregados a `Ubicacion` para ADR-067) no tienen ninguna fila en `TarifaFleteCamionproducto` ni en `TarifaRoundTrip` en el libro que motivó este ADR — `coberturaTarifas()` va a abortar la corrida pidiendo esos datos, que son un hueco de carga del libro, no de este código.

**Nota de implementación:** igual que ADR-065/066/067, el patch se aplicó directo sobre `RedLogistica_Exportacion.alp` con reemplazos de texto verificados como únicos antes de escribir, XML validado después de cada lote, y `model_src/`/`MANIFIESTO.md` regenerados. El diff queda contenido enteramente en `ImportadorExcel.java` — ninguna otra clase cambia, porque el diseño elegido (expandir buckets a filas `Tarifa` de siempre) evita tocar `DatosEntrada`. **No se compiló ni se corrió en AnyLogic.** Antes de dar la fase por terminada falta: correr una carga con `datos/entrada_ejemplo.xlsx` (formato original) para confirmar que el resultado es idéntico a antes de este ADR — es el caso de regresión obligatorio, dado que casi todo el cambio son ramas `if/else` que tienen que elegir siempre la rama vieja con ese libro —, y una carga con `Maestro_Simulacion.xlsx` una vez que el usuario complete las filas de flete/round trip de `GRUPO_PAZ`/`CONTROL_UNION` y decida qué hacer con Consolidado/Cross_docking/Gastos_THC.
