...
**Estado:** aceptada — implementada en `RedLogistica_Exportacion.alp` y `model_src/`; **pendiente de compilar y correr en el IDE de AnyLogic** (fuera de este entorno, sin motor de simulación disponible) para completar la "Definición de terminado" de `docs/07_Roadmap/Roadmap.md` §17 y ejecutar `V-COST-11`.
**Fecha:** 2026-08-08
...
**Nota de implementación:** el patch se aplicó directo sobre `RedLogistica_Exportacion.alp` (los cuatro cambios de la sección Decisión, ubicados por coincidencia exacta de texto y verificados de a uno como únicos en el archivo antes de escribir) y se regeneró `model_src/` con `tools/exportar_modelo.py`; el diff resultante toca únicamente `AlternativaCircuito.java` (el campo y el método nuevos) y `Main.java` (las dos funciones nuevas, el cálculo agregado en `costearAlternativa()` y el reemplazo puntual en `ordenarAlternativas()` — los otros seis usos de `costoUnitarioSegun()` en el archivo, todos de reporte/KPI, quedan intactos). Lo que **no** se verificó en este entorno, por no contar con el IDE de AnyLogic ni su motor de simulación: que el código compile dentro de AnyLogic, y la corrida empírica de `V-COST-11` (comparar `diasEstimadosAlmacenamiento = 0` contra el valor default y confirmar equivalencia byte a byte, más una corrida con política de costo para confirmar el cambio de ranking descrito en `flow/02-logica-entrega-pedidos.md` §2.6). Falta abrir el modelo en el IDE, compilar, y correr esas dos comparaciones antes de dar la fase por terminada según `docs/07_Roadmap/Roadmap.md` §17.

## ADR-066 — Afinidad pedido-depósito y rebalanceo entre depósitos

**Estado:** aceptada — implementada en `RedLogistica_Exportacion.alp` y `model_src/`; **pendiente de compilar y correr en el IDE de AnyLogic** (mismo estado que ADR-065 — sin motor de simulación en el entorno donde se implementó, ver nota de implementación al final).
**Fecha:** 2026-08-08
**Contexto:** ADR-065 corrigió que el evaluador de circuitos comparara costo de despacho incluyendo el holding futuro, pero corriendo el modelo aparecieron dos huecos reales que ningún ajuste de costo resuelve, porque son de naturaleza distinta:

1. **El modelo no sabe cuándo un pedido "ya tiene dueño".** En la operación real, parte del stock que queda en un depósito lejano no está genuinamente disponible para competir: ya está posicionado ahí porque un cliente/pedido específico lo va a retirar de ese lugar. El modelo, al no tener ese dato, lo deja competir en igualdad de condiciones contra todos los pedidos — puede perder esa competencia contra uno más barato en otro lado y quedar "atrapado" en la contabilidad del modelo, cuando en la realidad no está atrapado en absoluto.
2. **No existe movimiento físico entre depósitos.** `generarAlternativas()` ya trae una alternativa de "transferencia depósito-depósito", pero se agrega **descartada a propósito** (`TRANSFERENCIA_DEPOSITO_DEPOSITO`, "sin movimiento físico en el modelo", C7), y `docs/06_Validacion/Plan_de_Validacion.md` (V-COST-06) ya dejó documentada la fórmula de costo sin implementarla, "pendiente por decisión del usuario". Esto importa en concreto para depósitos que no tienen capacidad de salida propia: `BOREAS` tiene `posiciones_cross_dock = 0` y sólo 3 contenedores/día, contra 16 posiciones y 10 contenedores/día de `RUTA9` en el mismo escenario de ejemplo — Boreas puede tener stock perfectamente vendible y aun así no tener cómo sacarlo a tiempo.

**Decisión:** dos mecanismos, cada uno resolviendo el hueco que le corresponde — no se mezclan entre sí ni con el crédito de holding de ADR-065.

### D1. Afinidad pedido-depósito

1. **Reactivar `Pedido.depositoAsignado`, no como preferencia — como dato nuevo.** Ese campo existe en el esquema pero sólo se **escribe** después de asignar un pedido (registro histórico); nunca se lee para decidir. Se agrega un campo nuevo y distinto, `depositoComprometido` (`Pedido`, `DatosEntrada.PedidoPlan`), para no pisarle el significado a un campo que ya cumple otra función.
2. **Columna opcional en `PedidoPlan`: `deposito_comprometido`.** Vacía por defecto (`f.textoOpcional`, mismo patrón que `buque`/`viaje_buque`) — un libro sin esa columna sigue funcionando exactamente igual que antes.
3. **Un nuevo criterio en `ordenarAlternativas()`, entre servicio y frío propio.** Si el pedido trae un depósito comprometido, la alternativa cuyo origen coincide gana **mientras sea factible**, antes de mirar costo — mismo mecanismo que ya usa `PRIORIDAD_FRIO_PROPIO` para preferir planta, aplicado ahora a un origen específico por pedido. Si esa alternativa no es factible ese día (sin stock, sin capacidad), el criterio no distingue nada entre las demás y el pedido compite normalmente por las otras — no hay fallback especial que armar, es una consecuencia de cómo ya funciona el comparador.

### D2. Rebalanceo entre depósitos

4. **Se activa la transferencia depósito-depósito, pero no como alternativa del evaluador de pedidos — como extensión del lado de oferta.** No compite por costo contra el resto de las alternativas de un pedido (eso ya lo decide ADR-065 razonablemente bien); resuelve el problema de **capacidad de salida**, que es previo a cualquier pedido. Vive junto a `revisarTransferenciasPlanta()`, no junto a `generarAlternativas()`.
5. **Disparador: sin cross dock propio, y stock que ya lleva el horizonte de holding sin moverse.** `revisarRebalanceoEntreDepositos()` corre todos los días (paso 6b, después de `revisarTransferenciasPlanta`) y sólo mira depósitos con `datos.capacidadCrossDockDia(origen) <= 0` — no es "depósito caro", es "depósito sin forma de sacarlo a tiempo". Reusa `diasEstimadosAlmacenamiento` (ADR-056/065, una sola fuente de verdad) como umbral de antigüedad: no reubica stock que recién llegó y todavía puede ganar una competencia normal.
6. **Destino: `mejorDestinoRebalanceo()`, mismo criterio de costo que `seleccionarDeposito()`.** Flete de reubicación más el holding proyectado en destino (`horizonteHoldingEvitado()`, la misma función de ADR-065) — reutiliza, no reinventa.
7. **Costo según V-COST-06, ya documentado:** OUT del origen (`registrarOutDepositoTransferencia()`, nueva — el `registrarOutDeposito()` existente depende de un `Envio` que acá no existe), flete entre depósitos (`registrarFleteProducto()`, reusada tal cual — la categoría `FLETE_PRODUCTO` ya es agnóstica de ruta) e IN en el destino (`registrarInDeposito()`, reusada tal cual). Sin cargos de contenedor: el contenedor se arma después, en destino, por el camino que ya existe.
8. **Nuevo acumulador `costoFleteEntreDepositos`, sumado a la reconciliación de `FLETE_PRODUCTO`** junto a `costoFletePlantaDeposito` y `costoFleteGranelTerminal` — mezclarlo en el acumulador de planta habría etiquetado mal el KPI de "flete planta-depósito" del tablero.
9. **Simplificación declarada: no usa la agenda de flota multidiaria (ADR-061).** Usa la misma capacidad diaria agregada que el resto de las transferencias con esa agenda desactivada (`camionDiaViaje` calculado inline con `distanciaKmSimetrica`, no con `distanciaKm`, justamente para no reventar la corrida si falta el dato en una sola dirección). Con `habilita_flota_producto_multidiaria = true` (el default del escenario de ejemplo), esta es la única transferencia del modelo que no pasa por la agenda de camiones física — queda anotado como extensión pendiente, no como un descuido.
10. **Sin datos de tarifa/distancia depósito-depósito, no hace nada — no crashea ni asume costo cero.** `TarifaFleteProducto` y `Distancia` de `datos/entrada_ejemplo.xlsx` no traen ninguna fila depósito→depósito; `transferirEntreDepositos()` y `mejorDestinoRebalanceo()` chequean `hayTarifaFlete`/`hayTarifaSitio`/`distanciaKmSimetrica >= 0` antes de mover un gramo, y devuelven 0 si falta el dato — hace falta cargar esas filas para que el mecanismo tenga algún efecto.

**Alternativas:** modelar el rebalanceo como una alternativa más dentro de `generarAlternativas()`, compitiendo por costo contra las demás para un pedido puntual (descartada — el problema de Boreas no es de costo, es de capacidad de salida; forzarlo a competir por costo puede perder contra una alternativa más barata que igual no puede despacharse a tiempo); usar `distanciaKm()` (no simétrica) para la duración del viaje, reusando `camionDiaViaje()` tal cual (descartada — esa función explota con `RuntimeException` si falta la fila exacta origen→destino, y hoy no existe ninguna fila depósito→depósito; con `distanciaKmSimetrica()` el mecanismo queda inerte en vez de abortar la corrida); integrar de una el rebalanceo con la agenda de flota multidiaria (ADR-061) (descartada por alcance — es una generalización real de esa agenda a pares depósito-depósito, más trabajo del que este Mod puede validar sin poder compilar y correr en el entorno donde se implementó); dejar que el desempate de `PRIORIDAD_FRIO_PROPIO` y el de `depositoComprometido` compitan entre sí sin orden explícito (descartada — se define orden fijo: servicio, compromiso, frío propio, costo, para que dos corridas iguales decidan igual).

**Consecuencias:** con un pedido que trae `deposito_comprometido`, ese origen gana la competencia mientras tenga stock — el resultado del evaluador deja de ser "siempre el más barato" para pedidos con afinidad declarada, a propósito. `revisarRebalanceoEntreDepositos()` mueve stock de depósitos sin cross dock hacia depósitos con mejor capacidad de salida sólo si hay datos de tarifa/distancia cargados — sin esos datos (el caso de `datos/entrada_ejemplo.xlsx` hoy), el mecanismo queda escrito y activo pero no ejecuta ningún movimiento, lo cual es el comportamiento correcto y no un bug. `costoTotalCampania()` puede subir levemente donde el rebalanceo sí se ejecute (paga OUT + flete + IN por segunda vez para ese stock) a cambio de que se exporte en vez de quedar varado — es el mismo tipo de trade-off que V-COST-10 ya documentó para el rediseño completo del circuito. Ninguno de los dos mecanismos toca `costoIncremental`/`costoEndToEnd` de una alternativa de pedido existente, así que las reconciliaciones de ADR-052/053 sobre esas alternativas no deberían verse afectadas — sí cambia el total reconciliado de `FLETE_PRODUCTO` y `OUT_DEPOSITO`, que ahora incluye los cargos del rebalanceo, correctamente sumados a sus acumuladores.

**Nota de implementación:** igual que ADR-065, el patch se aplicó directo sobre `RedLogistica_Exportacion.alp` localizando cada bloque como único antes de escribir, XML verificado bien formado, `model_src/` y `MANIFIESTO.md` regenerados con `tools/exportar_modelo.py`. Se verificaron a mano, contra el código ya existente en el mismo archivo, todas las firmas reusadas (`hayTarifaFlete`, `hayTarifaSitio`, `distanciaKmSimetrica`, `espacioDisponibleEfectivo`, `registrarFleteProducto`, `registrarInDeposito`, `moverLote`, `tomarFlotaProducto`, `capacidadCrossDockDia`) y el balance de llaves de `Main.java`. **No se compiló ni se corrió en AnyLogic** — no hay motor de simulación en este entorno.

**Confirmado empíricamente (corrida del usuario, `E-00-R0`, post-implementación):** cero operaciones `REB-` en `costos_eventos.csv`, `decisiones_alternativas.csv`, `asignaciones_elegidas.csv` y `ejecucion_arcos.csv` — el mecanismo nunca se ejecutó. El stock final por depósito quedó idéntico al de antes de este ADR. Es el comportamiento esperado (§10 de la Decisión): `datos/entrada_ejemplo.xlsx` sigue sin ninguna fila depósito→depósito en `TarifaFleteProducto`/`Distancia`, así que `hayTarifaFlete()` corta antes de calcular ningún costo — **no se asumió ningún costo de flete, porque no se movió nada**. Se agregó una traza bajo `debugPlanificacion` en el punto donde `mejorDestinoRebalanceo()` devuelve `null`, para que la próxima corrida lo diga en la consola de AnyLogic sin depender de exportar y revisar los CSV de auditoría.

**Confirmado empíricamente que funciona con los datos cargados (corrida posterior, mismo `E-00-R0`):** con `posiciones_cross_dock = 0` en Boreas y Norry (revertido tras un cambio de datos que los había puesto en 10, lo cual —correctamente— desactivaba el disparador) y filas `BOREAS→RUTA9` / `NORRY→RUTA9` cargadas en `TarifaFleteProducto`/`Distancia`, el mecanismo movió **8.060 tn en 386 viajes** entre los días 30 y 346, y el stock de cierre en esos dos depósitos cayó de 2.446,7 tn a 21,8 tn (−99%). El costo del rebalanceo (flete + OUT + IN) fue USD 688.576, y el costo total de campaña **bajó** ~USD 155.000 respecto de la corrida sin rebalanceo (de ~6.397.129 a ~6.242.209 USD de caja) — el stock que antes quedaba varado se exportó, y esa ganancia compensó de sobra el costo de reubicarlo. Efecto secundario observado y esperable: al cambiar dónde termina el stock en la red, el evaluador de pedidos reordena levemente sus elecciones para el resto (Frinoa terminó con más stock que en la corrida de referencia, sin que el rebalanceo la haya tocado directamente).

Antes de dar la fase por terminada (`docs/07_Roadmap/Roadmap.md` §17) falta: compilar en el IDE (sigue sin haberse hecho en un entorno con AnyLogic real) y correr `V-COST-12` explícitamente (la afinidad pedido-depósito, D1, todavía no se probó con un pedido real con `deposito_comprometido` cargado).

## ADR-067 — Material como dimensión física del inventario

**Estado:** aceptada — implementada en `RedLogistica_Exportacion.alp` y `model_src/`; **pendiente de compilar y correr en el IDE de AnyLogic** (mismo estado que ADR-065/066, ver nota de implementación al final).
**Fecha:** 2026-08-09
**Contexto:** el nuevo maestro de datos (`Maestro_Simulacion.xlsx`) separa `Producto` (`JUGO`, `CASCARA`, `ACEITE` — el `TipoProducto` que el modelo ya conoce) de `Material`, una subdivisión productiva (para `JUGO`: `JCL`, `JCCL`, `PULPA`; `CASCARA` tiene `CDL`; `ACEITE` tiene `AEL`). El usuario describe el proceso real: los materiales se producen por separado y se exportan por separado, y confirma — al decidir entre las dos opciones planteadas — que **un pedido de un material no puede satisfacerse con stock de otro material del mismo producto**: no son sustituibles. Dos datos del maestro nuevo son evidencia de que esto no es sólo una etiqueta de trazabilidad:

1. **`Producto` trae capacidad de contenedor distinta por material**, no sólo por producto: `JUGO/JCL` y `JUGO/JCCL` usan `REEFER_40` de 24 tn, pero `JUGO/PULPA` usa el mismo `REEFER_40` con sólo 20 tn. Hoy `Main.obtenerCapacidadContenedorTon(TipoContenedor)` resuelve por tipo de contenedor buscando la primera fila de `Producto` que lo use — con dos materiales que comparten `REEFER_40` y difieren en capacidad, esa función devolvería la capacidad equivocada según el orden de la lista. No es una ambigüedad teórica: ya existe en los datos.
2. **`Stock_inicial` ya viene separado por material** (`JCCL`, `JCL`, `PCL`/`PULPA` como filas distintas con toneladas propias por depósito), mientras que `ProduccionPlan` todavía no tiene columna de material — confirmando que el material es un atributo real del stock físico, no sólo del pedido.

Se plantearon dos caminos (ver Alternativas): tratar `material` como un dato de trazabilidad del lote comercial, sin que participe en qué capa física satisface a un pedido (Opción A); o hacerlo participar en la selección física de inventario, igual que `producto` (Opción B). El usuario elige la Opción B explícitamente.

Sobre el punto 4 pendiente del análisis del maestro nuevo (`oportunidad_usd_tn_dia` / `penalidad_sobrecarga_usd_tn_dia`, la tarifa de oportunidad del frío propio de ADR-049 y la base de `costoHoldingEvitado` de ADR-065): el usuario decide dejarlo **sin efecto por ahora** — no hay columna equivalente en el maestro nuevo y no se agrega ninguna. Mientras se cargue este maestro, `tarifaHoldingOrigen("PLANTA", producto)` sigue devolviendo lo que traiga `TarifaSitio`/su reemplazo (hoy, ausente ⇒ 0 por `f.numero()`), así que el crédito de holding de ADR-065 queda estructuralmente en 0 para la planta hasta que se cargue ese dato explícitamente. No es un bug de este ADR: es una limitación de datos, documentada para no repetir el diagnóstico.

**Decisión:** `material` es una dimensión real del inventario, paralela a `producto` en todo el motor físico (`Capa`, `Inventario`, `LoteProducto`), pero la **capacidad física de almacenamiento sigue agregada por `TipoProducto`** (`CapacidadUbicacion` no tiene columna de material y no se le agrega: no hay dato de capacidad por material, y físicamente un tanque de jugo no separa sus materiales en compartimentos). Igual criterio para las tarifas de sitio (storage/in/out/THC/consolidación/despachante): el maestro nuevo no las varía por material, así que ninguna tarifa cambia de firma.

1. **`Capa` y `LoteProducto` ganan un campo `material` (String, default `""`)**, exactamente con el mismo patrón que `cliente`/`calidad` (ADR-047): vive en la identidad del lote y viaja con cada capa que ese lote genera. Un lote nunca cambia de material, igual que nunca cambia de producto.
2. **`Inventario` gana overloads material-aware** de las consultas y movimientos que hoy filtran sólo por `(idUbicacion, producto)`: `stock`, `libre`, `reservado`, `fifo`, `reservar`, `despachar`. Las firmas **existentes** (sin material) no se tocan y siguen agregando entre materiales — son las que usan capacidad, KPIs, transferencia planta→depósito preventiva (ADR-048) y rebalanceo entre depósitos (ADR-066): ninguna de esas decisiones depende de qué material es, sólo de cuánto producto hay y qué tan viejo es. `ingresar()` (creación de capa) pasa a pedir `material` siempre, porque toda capa nueva nace de un lote que ya lo tiene.
3. **Todo lo que resuelve stock físico *para un pedido puntual* pasa a filtrar por `pedido.material`.** Concretamente: `reservarParcialPedido()`, `alternativaPara()`/`toneladasDisponiblesParaAlternativa()`, `seleccionarSitioCrossDock()`, `ejecutarCrossDockPedido()` (y su `transferirLotesADeposito()`/`buscarLoteMasAntiguoEnPlanta()`, que ganan un overload con material), `depositosOrdenadosParaPedido()`/`costoEstimadoDesde()`, y las filas de auditoría de `costearAlternativa()` que reportan stock del origen. Las funciones que **no** reciben un pedido — `revisarTransferenciasPlanta()`, `toneladasASacarDePlanta()`/`toneladasASacarReactiva()`, `seleccionarDeposito()`, `revisarRebalanceoEntreDepositos()` — siguen operando en agregado por producto: mueven "la capa más vieja de este producto", que arrastra su propio material sin que la decisión necesite saberlo de antemano.
4. **`obtenerTipoContenedor`/`obtenerCapacidadContenedorTon` pasan a resolver por `(producto, material)` directo**, vía un nuevo `DatosEntrada.producto(TipoProducto, String material)` que reemplaza al `producto(TipoProducto)` de una sola clave (sus dos únicos llamadores migran). Se elimina así la ambigüedad descrita en el Contexto: ya no hay una búsqueda inversa por tipo de contenedor que pueda devolver la fila del material equivocado.
5. **`buscarLoteComercialAbierto()` agrega `material` al match**, en el mismo lugar donde ya compara `producto`, `cliente` y `calidad` — la producción diaria de un material sólo puede abrir o continuar un lote comercial de ese mismo material.
6. **`Planta.producir()` deja de leer un solo total por producto y por día**: itera los materiales de cada producto (`DatosEntrada.materialesDe(TipoProducto)`, derivado de la tabla `Producto`) y llama `ingresarProduccion(producto, material, toneladas)` por cada uno, usando el nuevo `DatosEntrada.produccionDelDia(dia, producto, material)`. La firma agregada por producto (`produccionDelDia(dia, producto)`) se mantiene para reportes que no necesitan el desglose.
7. **Contrato de datos:** `ProduccionPlan` y `PedidoPlan` ganan una columna opcional `material` (default `""`, mismo patrón que `deposito_comprometido`); `StockInicial` también, y `claveLote()` y la validación de "lote con productos distintos" (`DatosEntrada.validar()`) incorporan `material` a la clave — un `codigo_lote` no puede tener dos materiales, igual que no puede tener dos productos. `Producto` ya trae `material` en el maestro nuevo: deja de ser una fila por producto y pasa a ser una fila por `(producto, material)`, con `datos.materialesDe(producto)` derivando la lista de materiales válidos de esa misma tabla — no hay una tabla nueva que la duplique.
8. **Material vacío (`""`) es el material único de un producto que no distingue** (`CASCARA`/`CDL`, `ACEITE`/`AEL`, o cualquier corrida — sintética o con un Excel viejo — que no cargue la columna). El generador sintético sigue poblando una sola fila de `Producto` por `TipoProducto` con `material = ""`, así que el modo sintético no cambia de comportamiento: es exactamente la misma garantía de compatibilidad que ya usa `cliente_default`/`calidad_default`.
9. **`ContenedorExportacion` gana el campo `material`** (copiado de `pedido.material` al crear el contenedor, junto con `producto`) para que el documento de exportación pueda reportar el material real de cada contenedor — es lo que el usuario describe como "se exportan separados".

**Alternativas:** Opción A, material como dato de trazabilidad puro (como `cliente`/`calidad`), sin que participe en la selección física de inventario — descartada explícitamente por el usuario: contradice que los pedidos no acepten sustitución entre materiales, y no explica por qué `Stock_inicial` ya llega separado por material en el maestro nuevo. Agregar `material` como parte de la capacidad física (`CapacidadUbicacion` por producto **y** material) — descartada por falta de dato: el maestro nuevo no trae capacidad por material, y specular un reparto inventado sería peor que no modelarlo. Duplicar `Producto` en una tabla nueva `ProductoMaterial` en vez de agregarle una columna a la existente — descartada, redundante con lo que el maestro ya hizo (una fila por combinación). Propagar `material` también a las tarifas de sitio/flete/round-trip — descartada por ahora: ninguna hoja del maestro nuevo las varía por material, así que agregar la dimensión ahí sería anticipar un dato que no existe; si en el futuro el material sí tiene tarifa propia, es un ADR aparte.

**Consecuencias:** un pedido de `JUGO`/`JCCL` nunca puede reservar ni despachar stock de una capa de `JUGO`/`JCL`, aunque ambas estén en el mismo depósito y el modelo, antes de este ADR, las hubiera visto como el mismo stock. Esto puede **reducir** el nivel de servicio proyectado en escenarios donde la producción de un material va por detrás de sus pedidos, aunque sobre stock de otro material del mismo producto — es el comportamiento correcto, no una regresión: antes el modelo ocultaba ese déficit real prestando entre materiales que en la operación real no son intercambiables. `costoTotalCampania()` no cambia por este ADR en una corrida donde todo el material sea `""` (un solo material por producto, como hoy) — es el caso de regresión obligatorio. El crédito de holding de ADR-065 y el rebalanceo de ADR-066 siguen funcionando igual que antes, agregados por producto, porque ninguno de los dos decide en nombre de un pedido puntual.

**Reemplaza:** nada — extiende ADR-047 (identidad del lote comercial) con una tercera dimensión.

**Nota de implementación:** igual que ADR-065/066, el patch se aplicó directo sobre `RedLogistica_Exportacion.alp` con reemplazos de texto verificados como únicos antes de escribir, XML validado con `ElementTree` después de cada lote, y `model_src/`/`MANIFIESTO.md` regenerados con `tools/exportar_modelo.py`. A diferencia de ADR-065/066, esta vez varios de los cambios tocan **agentes reales** (`Pedido`, `LoteProducto`, `ContenedorExportacion`, `Planta`) y no sólo `JavaClasses`: el parámetro `material` se agregó como nodo `<Variable Class="Parameter">` con su propio `<Id>` (siguiendo el patrón exacto de `depositoComprometido`, ADR-066), y las funciones de `Main` cuya firma cambió (`obtenerTipoContenedor`, `obtenerCapacidadContenedorTon`, `contenedoresNecesarios`, `toneladasDisponiblesParaAlternativa`, `transferirLotesADeposito`, `buscarLoteComercialAbierto`, `crearLoteEnPlanta`, `toneladaDiaEnStock`, y la nueva `buscarLoteMasAntiguoEnPlanta(producto, material)`) necesitaron editar el nodo `<Parameter>` de la función además de su `<Body>`, porque en un agente (a diferencia de una `JavaClass`) la firma vive en XML estructurado y no sólo en el texto del cuerpo.

Se verificó exhaustivamente, con un script que cuenta la aridad de cada *call site* contra la definición de cada función tocada, que ningún llamador quedó con la firma vieja — encontró y corrigió dos que se habían escapado del primer paso (`toneladasDisponiblesParaAlternativa()` en `alternativaPara()`, y `inventario.ingresar()` en `recibirViajeProducto()`, donde `viaje.destino` habría caído en el parámetro `material` por el desplazamiento de posición). El segundo caso además dejó un patrón nuevo: como `ViajeProducto` no lleva su propio `material` (ADR-061 no lo necesitaba), `recibirViajeProducto()` lo resuelve buscando el lote por `viaje.idLote` y usando `""` si no hay lote asociado — mismo sentinel que el resto del modelo.

**Alcance deliberadamente no cubierto** (documentado para no repetirlo como bug): `programarViajeProducto()`/`iniciarViajeProducto()` tienen una rama de reserva/despacho sin lote (`idLote <= 0`) que sigue usando los overloads agregados de `Inventario` (sin material). Se relevaron los dos únicos llamadores reales de esa rama (`programarTransferenciaLote()` siempre pasa un lote real; `ocuparFlotaParaGranel()` siempre pasa `ocupaSoloFlota = true`, que salta esa rama por completo) y en ningún caso hoy se ejecuta sin lote — es código con una rama muerta preexistente a este ADR, no una laguna que este ADR introduce. Tampoco se propagó `material` a ninguna tarifa (`TarifaSitio`, `TarifaFleteProducto`, `TarifaRoundTrip`, `TarifaEspera`): ninguna hoja del maestro nuevo las varía por material, así que agregar la dimensión ahí habría sido anticipar un dato que no existe.

**No se compiló ni se corrió en AnyLogic** — no hay motor de simulación en este entorno. Antes de dar la fase por terminada falta: compilar en el IDE, correr el caso de regresión con `material = ""` en todo el libro (tiene que dar exactamente igual que antes de este ADR), y cargar un libro con materiales reales distintos por producto para confirmar que un pedido de un material no consume stock de otro (ver `V-COST-14`, `docs/06_Validacion/Plan_de_Validacion.md`).

## ADR-068 — El importador acepta el formato de tarifas del maestro nuevo, sin abandonar el contrato original

**Estado:** aceptada — implementada en `RedLogistica_Exportacion.alp` y `model_src/`; **pendiente de compilar y correr en el IDE de AnyLogic** (mismo estado que ADR-065/066/067, ver nota de implementación al final).
**Fecha:** 2026-08-09 (seguimiento de Consolidado/Cross_docking/Gastos_THC: 2026-08-10)
**Contexto:** además de agregar `material` (ADR-067), `Maestro_Simulacion.xlsx` cambia la forma de las hojas de tarifa: `TarifaFleteProducto` se renombra `TarifaFleteCamionproducto`; `TarifaSitio` se parte en `Consolidado`, `Cross_docking`, `Tarifa_almacenaje`, `Despachante`, `Gastos_THC` y `Gastos_terminal`; `TarifaRoundTrip` conserva el nombre pero cambia las columnas; y todas reemplazan `vigencia_desde`/`vigencia_hasta` + una columna `tarifa` por 12 columnas de mes (`'0-31'` … `'334-365'`) más una columna `Moneda`, con una hoja nueva `Tipo_cambio` para convertir a USD lo que viene en pesos. `ImportadorExcel` sólo sabía leer el contrato original: con este libro tal cual, la carga aborta con "Falta la hoja TarifaSitio" y "Falta la hoja TarifaFleteProducto" antes de llegar a ejecutar nada.

De los conceptos que `TarifaSitio` unificaba, tres tenían una ambigüedad real al migrar: `Consolidado`/`Cross_docking` (¿por sitio y producto, igual que siempre, o también por terminal de destino?) y `Gastos_THC` (¿por sitio, o por naviera?). Se preguntó explícitamente al usuario y, en una primera vuelta, la pregunta no se respondió — quedaron sin leer, con `advertencias` explícitas (ver la versión original de este ADR en el historial de git). En la vuelta siguiente el usuario contestó las tres: *"tambien adpte consolidado y cross como estaba origianalmente sin depender de la terminal, y gastos THC si depende de la naviera, porque es el costo de THC de la maritima"*. El libro se actualizó acorde (`TarifaConsolidado`/`TarifaCross_docking` sin columna `Terminal`, `Gastos_THC` sin cambios) y este ADR se actualiza para leer las tres.

**Decisión:** `ImportadorExcel` detecta el formato de cada hoja (por nombre de hoja o por columna presente) y lee cualquiera de los dos sin tocar `DatosEntrada`: la resolución de tarifa por día de campaña (`Tarifa.vigente()`, `tarifaSitio()`, etc.) sigue exactamente igual que hoy.

1. **`filas(hoja, columnaFiltro, valor)` filtra sólo si la columna existe.** Antes, filtrar por `id_escenario` en una hoja que no lo tiene agregaba "Falta la columna id_escenario" y abortaba la carga. Con el chequeo de `encabezados.containsKey(columnaFiltro)` antes de filtrar, una hoja del contrato original (sin `id_escenario`) se sigue leyendo entera, y una del maestro nuevo se filtra por escenario — mismo método, los dos formatos.
2. **Buckets mensuales de calendario, no de campaña.** Las 12 columnas de cada hoja repiten el mismo texto con imprecisiones de ±1 día entre hojas (`'60-89'` en una, lo mismo etiquetado distinto en otra); en vez de parsear el texto, `COLUMNAS_MES`/`DIA_DESDE_MES`/`DIA_HASTA_MES` fijan los 12 tramos de un año no bisiesto (0-364) y la columna manda por **posición**, no por lo que dice el encabezado. Cada fila de bucket se expande a 12 filas `Tarifa` (una por mes, con su propio `vigenciaDesde`/`vigenciaHasta`) en el momento de leer — `DatosEntrada` nunca se entera de que el origen era un bucket.
3. **Conversión de moneda al leer, no al resolver.** `Tipo_cambio` (mismos 12 buckets, `id_escenario`) da el tipo de cambio ARS/USD por mes; `aUsd(valor, moneda, mes, tipoCambio)` convierte `valor / tipoCambio[mes]` para toda fila cuya `Moneda` no sea exactamente `"USD"` — incluida `"$"`, que es como el maestro nuevo marca los pesos. La tarifa que llega a `DatosEntrada` ya está en USD, igual que hoy.
4. **`TarifaFleteCamionproducto`/`TarifaRoundTrip` con formato nuevo**: sin `variable_usd_tn` (se asume 0) y con `unidad = 'VIAJE'`/`'TN'` en vez de `USD_VIAJE`/`USD_TN` (`unidadDeViaje()` normaliza). Se detectan por nombre de hoja (`TarifaFleteProducto` vs `TarifaFleteCamionproducto`) o por columna presente (`TarifaRoundTrip.tiene("tarifa_usd_contenedor")`, mismo nombre de hoja en los dos formatos).
5. **`TarifaSitio` ausente ⇒ se arma desde `Tarifa_almacenaje` + `Gastos_terminal` + `Despachante`.** `leerTarifaSitioMaestroNuevo()` acumula en una matriz por `(idUbicacion, producto)` — `Tarifa_almacenaje` da `in`/`storage`/`out` directo (`Concepto` = `In`/`Storage`/`Out`, `Proveedor` = sitio, `Tipo` = producto); `Gastos_terminal` y `Despachante` no traen producto, sólo tipo de contenedor (`'40 HC'`/`'20 dry'`/`'40 RF'` en un caso, el nombre del enum en el otro — `contenedorOpcional()` normaliza los dos), así que se resuelven contra `productoDeContenedor()`, que asume un tipo de contenedor por producto (vale para `JUGO`/`CASCARA`/`ACEITE` con `REEFER_40`/`DRY_HC_40`/`IMO_DRY_20`, que es lo que hay). Un sitio que el maestro nombra distinto al `id_ubicacion` de `Ubicacion` (`EXOLGAN`, `TPROSARIO`, `TRPLATA` no existen ahí) se descarta con `advertencias`, no con un error que aborte todo el libro por un dato que de todos modos no se puede usar.
6. **`Consolidado` y `Cross_docking` entran al mismo acumulador que `Tarifa_almacenaje`/`Gastos_terminal`/`Despachante`, confirmado sin dependencia de terminal.** El acumulador de `leerTarifaSitioMaestroNuevo()` pasa de 5 a 7 conceptos por `(idUbicacion, producto)`; `TarifaConsolidado` (o `Consolidado`, tolerando el nombre sin el prefijo del maestro 2026) y `TarifaCross_docking` (o `Cross_docking`) se leen con la misma técnica que `Gastos_terminal`/`Despachante`: `Lugar Consolidado` resuelve el sitio, `Tipo de Contenedor` resuelve el producto vía `productoDeContenedor()`. `consolidacion_tarifa` y `cross_dock_tarifa` de `TarifaSitio` dejan de ser siempre `0`.
7. **`Gastos_THC` no encaja en `TarifaSitio` — es una tabla nueva, `TarifaThc`, con clave (naviera, tipo de contenedor).** Confirmado por el usuario que el costo lo factura la naviera, no el sitio, así que no tiene sentido forzarlo en el acumulador de arriba (que resuelve por `idUbicacion`). `DatosEntrada` gana la clase `TarifaThc extends Tarifa`, la lista `tarifasThc`, el resolvedor `tarifaThc(dia, naviera, tipoContenedor)` y el overload `thcUsdContenedor(dia, naviera, tipoContenedor)` — la firma original `thcUsdContenedor(dia, idTerminal, producto)` no se toca, así que el contrato viejo sigue resolviendo exactamente igual. `ImportadorExcel.leerGastosThc()` lee la hoja hacia `datos.tarifasThc`; proveedores que no son una naviera real del enum `Naviera` (`FORWARDER`, `SILVERFREIGHT`: son forwarders/alternativas, no líneas marítimas) se descartan con `advertencias` en vez de abortar el import — en los datos reales, el 100% de `PedidoPlan` declara `naviera = MAERSK`, que sí está en el enum, así que esas dos filas quedan como dato inerte, no como bloqueo. `Main.java` gana `thcUsdContenedorPedido(dia, pedido, terminal)`: si `datos.tarifasThc` no está vacía resuelve por naviera con el tipo de contenedor calculado *fresco* vía `obtenerTipoContenedor(pedido.producto, pedido.material)` (no lee `pedido.tipoContenedor`, que el `StartupCode` deja resuelto con los valores default del agente y sólo queda correcto mucho más tarde, en `crearContenedoresParaAsignacion()` — no confiable en los tres puntos donde se cobra THC, que corren antes); si está vacía, cae al `thcUsdContenedor` de siempre. Los tres call sites (`registrarCargosTerminal`, la auditoría de `costoEsperadoCircuito` y `costearAlternativa`) pasan por este helper.

**Bug encontrado y corregido de paso (no de este ADR, sino de ADR-067):** `coberturaTarifas()` y `unaSolaRoundTrip()` llamaban `producto(producto).tipoContenedor` con un solo argumento, pero `DatosEntrada.producto()` sólo existe de a dos (`producto, material`) desde ADR-067 — no compilaba. Se agregó `tipoContenedorDe(producto)`, que resuelve el tipo de contenedor vía el primer material de `materialesDe(producto)` y **falla explícitamente** si dos materiales del mismo producto declaran tipos de contenedor distintos (no pasa con los datos reales: `JCL`/`JCCL`/`PCL` de `JUGO` comparten `REEFER_40`, sólo cambia la capacidad). Los tres usos de `producto(producto).tipoContenedor` (dos preexistentes más el que agregó la cobertura de THC de este ADR) se reemplazaron por `tipoContenedorDe(producto)`.

**Alternativas:** convertir el libro nuevo al formato viejo con un script aparte antes de importar (descartada — dos formatos de verdad en paralelo, y cualquier corrección posterior en el maestro real habría que volver a convertir); resolver los buckets dentro de `DatosEntrada` en vez de expandirlos al leer (descartada — reescribe `Tarifa.vigente()` y `coberturaTarifas()` para una segunda representación de vigencia, sin necesidad: expandir a filas por mes reusa toda la lógica existente sin tocarla); forzar THC dentro de `TarifaSitio` agregando una columna `naviera` a esa tabla (descartada — `TarifaSitio` resuelve por `(idUbicacion, producto)`, y THC no depende de ninguno de los dos; hubiera significado una fila de `TarifaSitio` por naviera aunque el resto de sus columnas no varíen por naviera, o ignorar la naviera de las otras filas — una tabla nueva con su propia clave es más simple que forzar una que no calza); adivinar consolidación por terminal y THC por naviera con el criterio que parecía más razonable en la primera vuelta (descartada explícitamente — son cambios de modelo que afectan el costo real de la campaña, y la pregunta hecha al usuario no se respondió esa vez; se esperó la confirmación en vez de adivinar).

**Consecuencias:** con el libro `Maestro_Simulacion.xlsx` (ronda 3), la carga ya no aborta por hoja faltante ni deja Consolidado/Cross_docking/THC en `0`: las tres hojas se leen y afectan el costo real de la campaña. `GRUPO_PAZ` y `CONTROL_UNION` (agregados a `Ubicacion` para ADR-067) ya tienen filas en `TarifaFleteCamionproducto` y `TarifaRoundTrip` en la ronda 3 del libro — el hueco de cobertura que bloqueaba la ronda 2 quedó resuelto por el usuario, no por este código. La columna `material` de `ProduccionPlan`/`PedidoPlan`/`Producto` también llegó en minúscula en la ronda 3 (antes era `Material`, y `filas()` compara nombres de columna exacto), así que ya no hace falta la corrección pendiente que señalaba la versión anterior de este ADR.

**Nota de implementación:** igual que ADR-065/066/067, el patch se aplicó directo sobre `RedLogistica_Exportacion.alp` con reemplazos de texto verificados como únicos antes de escribir, XML validado después de cada lote, y `model_src/`/`MANIFIESTO.md` regenerados. El diff de este seguimiento toca `ImportadorExcel.java` (Consolidado/Cross_docking/Gastos_THC), `DatosEntrada.java` (`TarifaThc`, `tipoContenedorDe`, validaciones y cobertura) y `Main.java` (`thcUsdContenedorPedido` y sus tres call sites) — más que el ADR-068 original porque THC exigió una tabla nueva, no sólo una rama de lectura. **No se compiló ni se corrió en AnyLogic** (no hay motor de simulación disponible en este entorno); la verificación de esta vuelta fue: balance de llaves, XML válido, y un chequeo de aridad exhaustivo (script ad hoc) comparando cada definición de función contra todos sus call sites en `model_src/*.java`, igual técnica que ADR-067. Sigue pendiente: correr una carga con `datos/entrada_ejemplo.xlsx` (formato original) para confirmar que el resultado es idéntico a antes de este ADR, y correr `Maestro_Simulacion.xlsx` (ronda 3) en el IDE de AnyLogic — lo único que este entorno no puede hacer por sí mismo.

## ADR-069 — El maestro real corre entero: `Stock_inicial` con material, red del modelo como superconjunto y vigencia abierta al cierre

**Estado:** aceptada — implementada en `RedLogistica_Exportacion.alp` y `model_src/`, **compilada y corrida en AnyLogic PLE 8.9.9**: campaña completa de 365 días desde `datos/Maestro_Simulacion.xlsx` (`Finished`, sin excepciones) y desde `datos/entrada_ejemplo.xlsx` (regresión del contrato original).
**Fecha:** 2026-08-11
**Contexto:** ADR-065 a ADR-068 se escribieron sin motor de simulación disponible y quedaron declarados "pendiente de compilar y correr en el IDE". Al correrlos por primera vez con el maestro real, la carga y la campaña abortaron en ocho puntos distintos, ninguno de ellos previsto por esos ADR. Siete son del modelo y uno era una regresión abierta sobre el libro canónico: **`entrada_ejemplo.xlsx` ya no cargaba**, porque ADR-068 exigía la hoja `Tipo_cambio`, que sólo existe en el maestro nuevo.

**Decisión:** ocho correcciones, cada una en el punto donde el dato se resuelve y sin cambiar ninguna política económica.

1. **`Stock_inicial` se lee en el formato en que el relevamiento la produce.** El maestro nombra las toneladas `1-365` (un rango de días, no un alias de "toneladas") y repite el encabezado `Producto` para traer el material. `columnaBucketUnica()`/`esRangoDeDias()` reconocen la primera; `columnaRepetida()` la segunda. Las dos interpretaciones se anuncian con `advertencias` en la consola: convertir en silencio una columna de rango en toneladas es exactamente el tipo de suposición que después nadie puede auditar. Sin esto, las **6 589 tn** de stock inicial no entraban, y con `material = ""` habrían entrado inertes: desde ADR-067 ningún pedido puede consumirlas.
2. **El tipo de contenedor no se resuelve en el `StartupCode` del pedido ni del contenedor.** `Pedido.onStartup()` corre **antes** de que `crearPedido()` cargue producto y material, así que consultaba la tabla `Producto` con `material = ""` y la campaña abortaba en el primer pedido (`Falta la fila de JUGO/ en la tabla Producto`); `ContenedorExportacion.onStartup()` hacía lo mismo con `ACEITE/`. El pedido resuelve `tipoContenedor`/`capacidadContenedorTon` en `crearPedido()`, inmediatamente después de asignar identidad; el contenedor conserva su red de seguridad, pero sólo cotiza si ya vino con material.
3. **La red del modelo es un superconjunto de la red del libro, en los dos sentidos.** `GRUPO_PAZ` y `CONTROL_UNION` son depósitos habilitados del maestro y ahora tienen agente `Deposito` en `Main`, con su stock en el tablero. Al revés, un agente que el libro no declara se **saca de la colección `depositos`** al cargar los datos, con advertencia, en vez de quedar pidiendo capacidades y tarifas que ese escenario no tiene por qué traer (era lo que rompía `entrada_ejemplo.xlsx` en tres puntos consecutivos: `Ubicacion`, `CapacidadUbicacion` y `TarifaSitio`). Y si el libro declara un depósito habilitado sin agente, la carga **falla temprano y lo nombra**, en vez de aparecer 200 días después como "la ubicación no tiene capacidad para el producto".
4. **Un depósito sin capacidad declarada de un producto no necesita tarifas de ese producto.** `capacidadDeclaradaTn()`/`almacenaProducto()` distinguen "no declara capacidad" de "declara cero", y tanto `coberturaTarifas()` como `refrescarTarifasDelDia()` omiten esas combinaciones. `GRUPO_PAZ` y `CONTROL_UNION` son depósitos de cáscara: exigirles tarifa de jugo es pedir un dato que la corrida no va a leer nunca.
5. **El último tramo mensual del maestro rige hasta el cierre de la corrida.** Los 12 buckets cubren los días 0–364, pero el flujo sigue cerrando envíos después del último día de campaña (ADR-064) y esos cargos se cotizan con la tarifa del día de devengo: la campaña abortaba en el día 365 con `Falta la tarifa de JUGO en FRINOA vigente el día 365`. `hastaDelMes(11)` deja el último tramo abierto, que es el criterio que el propio maestro usa en `TarifaEspera` (último tramo hasta 9999).
6. **`Tipo_cambio` es opcional.** Sólo existe donde hay tarifas en pesos; sin la hoja el tipo de cambio queda en 1,0, que es lo que el contrato original ya asumía al cotizar todo en USD. Esto reabre el libro canónico, que desde ADR-068 no cargaba.
7. **Las cuatro distancias que faltaban son dato, no código**: `GRUPO_PAZ`/`CONTROL_UNION` → `ZARATE`/`T4`, a 1 200 km, confirmadas por el usuario, más las capacidades explícitas en cero de los productos que esos depósitos no almacenan y las tarifas de espera y flete de cáscara que el libro sí necesitaba.
8. **`tools/exportar_modelo.py` exporta también el `StartupCode` de cada agente.** Dos de los ocho errores estaban en código de arranque, y ese código no aparecía en `model_src/`: el espejo revisable no mostraba la línea que abortaba la campaña.

**Alternativas:** pedirle al usuario que renombre las columnas de `Stock_inicial` (descartada — el maestro es el entregable de un relevamiento, y la próxima ronda volvería a traerlas así); cargar el stock inicial sin material (descartada — con ADR-067 serían 6 589 tn que ningún pedido puede consumir, o peor, un jugo que tapa el déficit de otro material); agregar `GRUPO_PAZ`/`CONTROL_UNION` a `entrada_ejemplo.xlsx` para que la red del libro cubra la del modelo (descartada como solución del problema, no del síntoma: cada depósito nuevo del canvas obligaría a editar todos los libros; el escenario tiene que poder usar un subconjunto de la red); extender el último bucket sólo hasta 365 (descartada — el cierre de un envío puede caer varios días después del último día de campaña, y el número exacto depende del libro).

**Consecuencias:** el maestro real corre entero y sus números son medibles por primera vez. Campaña `E-00`, 365 días: 30 656 tn pedidas, **29 439 tn exportadas**, servicio 72 % por tonelada al cut-off, 16 pedidos atrasados con atraso medio 1,0 día, 8 187 074 USD de caja, auditoría `COMPLETA` con 50 496 alternativas evaluadas en 2 104 rondas, 10 775 arcos, 112 593 cargos y 3 780 snapshots de inventario con **descuadre 0**. El techo por dato que anunciaba el MOD se confirma medido: `JUGO/JCL` asigna **15 842 tn de 16 961 pedidas** —exactamente su disponibilidad de campaña— mientras `JCCL` y `PCL` entregan su demanda completa (142 y 133 tn) sin ser canibalizados: el material es una restricción física, no una etiqueta. Las 21 filas descartadas con advertencia (`Gastos_terminal`, `Despachante`, `Gastos_THC`) son terminales que el maestro nombra distinto a `Ubicacion` (`EXOLGAN`, `TPROSARIO`, `TRPLATA`, `TRP`) y proveedores que no son navieras (`FORWARDER`, `SILVERFREIGHT`): datos que la corrida no puede usar, no huecos de cobertura. **No se corrió ningún barrido**, por pedido explícito.
